  ...
  <div id="app"
    ...
    <el-tabs v-model="activeTabName"
      ...
      <el-tab-pane label="常用推荐"
        ...
        <ul class="tools">
          <li v-for="(group, groupIndex) in tools"
              :key="groupIndex"
              class="tools__item"
              :class="group.key"
              :data-index="groupIndex"
              @click="handleOpenDevelopPage">
            <span class="tools__item__icon"
                  :style="{ backgroundImage:`url(${group.icon})` }" />
            {{ group.title }}
          </li>
        </ul>
        <!-- 此处可加入贵公司的内推网址 -->
        <!-- <a class="button__neitui"
           :href="recommendationLink"
           target="_blank">一键内推</a> -->
      </el-tab-pane>
      ...
      <el-tab-pane lazy
        ...
        <h3 slot="label"
            class="app__target">
          <i class="el-icon-edit" /> 珠峰周刊
        </h3>
        <el-tabs v-model="weekly.activeName"
                 @tab-click="handelReadingTabsClick">
          <el-tab-pane label="周刊"
                       name="weekly">
            <el-carousel height="400px"
              ...
              <el-carousel-item>
                <ul class="weekly__list">
                  <li v-for="(item, itemIndex) in weekly.data"
                      :key="itemIndex"
                      class="weekly__item">
                    <el-card shadow="hover">
                      <a class="weekly__item__link"
                         :href="getWeeklyUrl({index:item.count, type:'pageLink'})"
                         target="_blank">
                        <img class="weekly__item__banner"
              ...
              <el-carousel-item v-for="(item, index) in otherBlogs"
                                :key="index">{{item}}
                <iframe :src="item"
                        frameborder="0"
                        width="368"
          ...
          <el-tab-pane label="投稿"
            ...
            <section class="article-recommend">
              ...
              <div class="article-recommend__item">
                <h4>○ 投稿人昵称</h4>
                <!-- <el-select v-model="article.reporter"
                           filterable
                           placeholder="请选择昵称">
                  <el-option v-for="item in originData.reporterOptions"
                             :key="item.value"
                             :label="item.value"
                             :value="item.value" />
                </el-select> -->
                <el-input v-model="article.reporter"
                          placeholder="请输入昵称" />
              </div>
              <div class="article-recommend__item">
                <h4>○ 设置分类</h4>
                <el-cascader v-model="article.category"
                             expand-trigger="hover"
                             :options="originData.articleCategories"
                             :show-all-levels="false" />

                <el-button class="article-recommend__button--submit"
                           type="primary"
...
.button__neitui {
  ...
  background: url("https://sitecdn.zcycdn.com/zcy-front-other-upload/777d611d9ca74be9033162909ef8bc6f.png")
    no-repeat 0 0;
